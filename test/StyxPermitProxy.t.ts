...
const ZeroExchangeProxy = "0xdef1c0ded9bec7f1a1670819833240f027b25eff"; // we use mainnet because arb forking is wonky
const FireBirdRouter = "0xe0C38b2a8D09aAD53f1C67734B9A95E43d5981c0";
...
describe("Styx Permit2 Proxy", function () {
    async function fixtures() {
        ...
        // Setup routers
        await ownerHelper.setAggregator(permitProxy.address, ZeroExchangeProxy, "0")
        await ownerHelper.setAggregator(permitProxy.address, OneInchRouter, "1")
        await ownerHelper.setAggregator(permitProxy.address, FireBirdRouter, "2")
    ...
    // it("Should execute a token to token trade using 1inch", async function () {
    //     const { tokenIn, tokenOut, arbAddressTable, utilsPermit, permitProxy, addr1 } = await loadFixture(fixtures);
    ...
    //     const quoteResponse = await fetch(
    //         `https://api.1inch.io/v4.0/1/swap?fromTokenAddress=${tokenIn.address}&toTokenAddress=${tokenOut.address}&amount=${amountInUncompressed}&slippage=${0.5}&disableEstimate=true&fromAddress=${permitProxy.address}`
    //     );
    ...
    //         witnessTypeName: "Witness",
    //         witnessType: { Witness: [{ name: "swapCalldata", type: "bytes32" }] },
    //         witness: { swapCalldata: ethers.utils.keccak256(quote.tx.data) }
    //     }
    ...
    //     const packedData = ethers.BigNumber.from("1") // routerId
    //         .shl(112)
    //         .or(ethers.BigNumber.from(amountInCint).shl(48))
    //         .or(ethers.BigNumber.from(tokenInIndex).shl(24))
    //         .or(ethers.BigNumber.from(tokenOutIndex));
